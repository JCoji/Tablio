...
def create_mixed_guitarset(
    ...
    musdb_tracks = list(musdb_dir.glob('*/'))
    
    if not musdb_tracks:
        raise ValueError(f"No MUSDB18 tracks found in {musdb_dir}")
    
    print(f"Found {len(musdb_tracks)} MUSDB18 tracks")
    ...
    process_split(guitarset, train_ids, musdb_tracks, train_dir)
    
    # Process validation set
    print("\n📦 Processing validation set...")
    process_split(guitarset, val_ids, musdb_tracks, val_dir)
...
def process_split(guitarset, track_ids, musdb_tracks, output_dir):
    ...
    for i, track_id in enumerate(tqdm(track_ids, desc="Mixing tracks")):
        try:
            ...
            guitar_audio, sr = sf.read(gt_track.audio_hex_comb_path)
            ground_truth_jams = gt_track.jams
            ...
            ground_truth_jams.save(output_jams_path)
